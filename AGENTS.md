...
15. **Стандарты вставки и отображения изображений в документации (`backlog/docs/`, `backlog/decisions/`)**:
    - Использовать стандартный Markdown-синтаксис `![Описание](URL_или_data_URI)` либо теги `<img src="..." alt="..." width="..." height="..." />`.
    - Не допускать конструкций, превращающих картинки в обычные текстовые ссылки.
    - Встроенные парсеры Markdown в UI (`MarkdownViewer`) обязаны корректно отображать как синтаксис `![alt](src)`, так и HTML-теги `<img>` с атрибутами `width`/`height` внутри таблиц и блоков.
