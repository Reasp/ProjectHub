...
20. **Управление компьютером — только через прокси ProjectHub и только по необходимости (decision-27, TASK-82)**:
    - Инструменты `computer_*` подключаются фичей шаблона `computerUse` (по умолчанию выключена) — stdio-мостом
      `scripts/computer-use/computer-use-bridge.mjs` к встроенному MCP-серверу ProjectHub. Рантайм
      `@zavora-ai/computer-use-mcp` напрямую в `.mcp.json`/`.agents/mcp_config.json` не подключать: HITL, allowlist,
      аудит и kill-switch тогда зависели бы от движка. Токен — только в переменной окружения `PROJECTHUB_MCP_TOKEN`,
      в конфиги не писать.
    - Порядок работы — accessibility-first: `computer_list_windows` → `computer_get_ui_tree`/`computer_find_element` →
      действия по элементам (`computer_click_element`, `computer_set_value`, `computer_press_button`); скриншот — для
      проверки результата; координатные клики — крайний случай. Координаты со скриншота передавать с
      `coordinate_space: "screenshot"`.
    - Отказ пользователя, отказ политики или сработавший kill-switch не обходить другими инструментами (скрипты,
      `run_command`, файловые операции) — остановиться и сообщить пользователю.
