...
14. **Обязательная полная сборка Portable десктопного приложения (Full Portable Rebuild)**:
    - После любых изменений в исходном коде `ProjectHub` агент обязан всегда выполнять полную сборку десктопного приложения в единый автономный переносимый файл через `npm run dist:win`, чтобы обновлять исполняемый бинарник в каталоге `release/` (`ProjectHub <version>.exe` и `release/win-unpacked/`), а не ограничиваться только компиляцией фронтенда `npm run build`.
