...
14. **Обязательная полная пересборка десктопного приложения (Full Rebuild)**:
    - После любых изменений в исходном коде `ProjectHub` агент обязан всегда выполнять полную сборку десктопного приложения через `npm run pack:win` (или соответствующую команду упаковки), чтобы обновлять исполняемый бинарник и файлы в каталоге `release/win-unpacked/`, а не ограничиваться только компиляцией фронтенда `npm run build`.
