...
Автосудья Swarm Arena реализован полностью (decision-12 → `accepted`, механика зафиксирована в decision-20).

Проверки из `.projecthub.json` (или выведенные из `package.json` scripts) прогоняются в worktree каждого кандидата через новый одноразовый `processManager.runOnce` — с таймаутом, отменой по `AbortSignal`, лимитом вывода и изоляцией портов, без засорения вкладки Processes. Собираются метрики диффа (модули локально, зависимые символы через `gitnexus detect-changes`), роль `reviewer` даёт структурированный отзыв с вердиктом по каждому критерию приёмки задачи, и всё складывается в балл 0–100 с разложением по семи компонентам. Блокирующая проваленная проверка запрещает рекомендацию и авто-мердж; авто-мердж выключен по умолчанию, требует зелёных проверок и балла выше порога и пишется в аудит HITL. Результат можно собрать из файлов нескольких кандидатов.

Автозапуск судьи ограничен настоящей ареной (>1 кандидата, `origin !== 'assigned'`): у одиночного агента сравнивать не с кем, а проверки и платное ревью были бы неожиданной тратой — для него осталась кнопка в UI.

Коммит: 9f83032 (38 файлов, +5292/−19). Проверено: `npm run lint` — 0 ошибок (510 предупреждений, baseline не вырос), `npm test` — 588 тестов зелёные в трёх полных прогонах подряд, `npm run build` и `npm run pack:win` проходят, распакованное приложение стартует, `npm run lint:docs` чист, RAG-индекс пересобран.
