...
19. **Обязательная полная сборка Portable десктопного приложения**: Введено Правило 14 (`infra-dev.md`), обязывающее выполнять полную пересборку в переносимый `.exe` (`npm run dist:win`).
