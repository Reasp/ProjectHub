...
> **Статус**: Все 14 базовых модулей реализованы и протестированы. Проект успешно собирается (`npm run dist:win`).  
...
15. **Брендовая айдентика и сборка**: Сгенерирована официальная иконка (Neon Cyber Nexus) без рамок и надписей с повышенным контрастом в многослойном `.ico` и `.png` (256/128/64/48/32/16px), настроена иконка исполняемого файла `.exe`, заголовок окна и логотип в приложении.
16. **Стандарты документации Backlog.md и вставки изображений**: Введена строгая валидация формата документов (`doc-<id> - <Title>.md`) и решений (`decision-<id> - <Title>.md`), а также единые стандарты вставки изображений (`infra-dev.md`, Правила 13 и 15).
17. **Обязательная полная сборка Portable десктопного приложения**: Введено Правило 14 (`infra-dev.md`), обязывающее выполнять полную пересборку в переносимый `.exe` (`npm run dist:win`).
