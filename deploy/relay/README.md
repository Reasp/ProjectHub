...
- Каталог: `GET /api/federation/hosts` — список аутентифицированных (прошедших подпись) хостов,
  подключённых прямо сейчас. Требует `Authorization: Bearer $RELAY_API_TOKEN`; без заданного
  `RELAY_API_TOKEN` каталог выключен целиком (он раскрывает имена машин, а relay стоит в открытом
  интернете).
