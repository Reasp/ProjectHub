      ...
      <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-6">
        ...
        {milestones.length > 0 && (
          <div className="grid grid-cols-3 gap-4">
            <div className="p-4 rounded-xl bg-[#161922]/80 border border-slate-800 flex items-center justify-between">
              <div>
                <p className="text-[11px] text-slate-400 font-medium">Всего этапов</p>
            ...
            <div className="p-4 rounded-xl bg-[#161922]/80 border border-slate-800 flex items-center justify-between">
              <div>
                <p className="text-[11px] text-slate-400 font-medium">В процессе разработки</p>
            ...
            <div className="p-4 rounded-xl bg-[#161922]/80 border border-slate-800 flex items-center justify-between">
              <div>
                <p className="text-[11px] text-slate-400 font-medium">Успешно завершено</p>
        ...
        {milestones.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center p-12 text-center border border-dashed border-slate-800 rounded-2xl bg-[#161922]/40">
            ...
            <h3 className="text-sm font-semibold text-white mb-1">Нет созданных майлстоунов</h3>
            <p className="text-xs text-slate-400 max-w-sm mb-4">
              Разделите разработку проекта на контрольные этапы, спринты или версии (v1.0, MVP, Phase 2).
            </p>
            <button
              onClick={handleOpenCreate}
          ...
          <div className="flex flex-col gap-4">
            {milestones.map((m) => {
              const mTasks = tasks.filter(
                (t) =>
                  t.milestone?.toLowerCase() === m.id.toLowerCase() ||
              ...
              const inProgressCount = mTasks.filter((t) => t.status === 'In Progress').length;
              const reviewCount = mTasks.filter((t) => t.status === 'Review').length;
              const todoCount = mTasks.filter((t) => t.status === 'To Do').length;
                ...
                >
                    ...
                    <div className="flex items-start justify-between gap-4">
                      ...
                      <div className="flex items-center gap-1.5 shrink-0">
                        <button
                          ...
                          title="Открыть задачи этого майлстоуна на Канбан-доске"
                        >
                        ...
                        <div className="flex items-center gap-3 text-slate-400 text-[11px]">
                          {m.targetDate && (
                            <span className="flex items-center gap-1">
                              ...
                              Дедлайн: <span className="text-slate-200 font-mono">{m.targetDate}</span>
                            </span>
                          )}
                          <span className="flex items-center gap-1">
                            <ListTodo className="w-3.5 h-3.5 text-slate-400" />
                            Задачи: {doneCount} из {totalCount} закрыто
                          </span>
                        </div>
                  ...
                  <div className="px-5 py-2.5 bg-[#12151e]/60 border-t border-slate-800/80 flex items-center justify-between text-xs">
                    ...
                    >
                      ...
                      />
                      {isExpanded ? 'Скрыть список задач' : `Показать задачи (${totalCount})`}
                    </button>
                    ...
                    <div className="p-4 bg-[#10131b] border-t border-slate-800/80 flex flex-col gap-2">
                      {mTasks.length === 0 ? (
                        <p className="text-xs text-slate-500 italic py-2 text-center">
                          Нет привязанных задач. Откройте карточку любой задачи и выберите этот майлстоун в выпадающем списке.
                        </p>
                      ) : (
                        mTasks.map((task) => (
                          <div
