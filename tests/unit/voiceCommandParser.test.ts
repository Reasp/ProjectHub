...
describe('parseVoiceCommand: управление агентом', () => {
  ...
  it('знак препинания после обращения не попадает в текст промпта (TASK-96)', () => {
    const cases: Array<[string, string]> = [
      ['Напиши агенту, проверь тесты.', 'проверь тесты.'],
      ['Скажи агенту: запусти сборку', 'запусти сборку'],
      ['Отправь агенту — обнови документацию', 'обнови документацию'],
      ['Отправь агенту – обнови документацию', 'обнови документацию'],
      ['напиши агенту,проверь тесты', 'проверь тесты'],
      ['Напиши агенту. Проверь тесты.', 'Проверь тесты.'],
      ['Промпт: добавь логирование', 'добавь логирование'],
      ['Ask agent, run the linter', 'run the linter'],
      ['Спроси, что сломалось', 'что сломалось'],
      ['Напиши, агенту проверь тесты', 'агенту проверь тесты']
    ];
    for (const [phrase, text] of cases) {
      expect(parseVoiceCommand(phrase), phrase).toMatchObject({ intent: 'send_prompt', payload: { text } });
    }
  });

  it('обращение без текста не отправляет пустой промпт', () => {
    for (const phrase of ['Напиши агенту.', 'Скажи агенту: ', 'Отправь агенту —']) {
      expect(parseVoiceCommand(phrase)?.intent, phrase).not.toBe('send_prompt');
    }
  });
});
